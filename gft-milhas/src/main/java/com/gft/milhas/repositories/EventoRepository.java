...
public class EventoRepository implements IEventoRepository {
	...
	public List<Evento> findAll(Date dataMin, Date dataMax) throws Exception {
		...
		PreparedStatement statement = connection
				.prepareStatement("select * from evento where data_inicio between ? and ?");
	...
	@Override
	public List<String> findEventosPorNome() throws Exception {
		Connection connection = ConnectionFactory.getConnection();

		PreparedStatement statement = connection.prepareStatement("select nome from evento");
		ResultSet resultSet = statement.executeQuery();

		List<String> nomesEventos = new ArrayList<String>();
		while (resultSet.next()) {
	...
	@Override
	public List<Integer> findEventosPorId() throws Exception {
		Connection connection = ConnectionFactory.getConnection();

		PreparedStatement statement = connection.prepareStatement("select idevento from evento");
		ResultSet resultSet = statement.executeQuery();

		List<Integer> listaIdEventos = new ArrayList<Integer>();
		while (resultSet.next()) {
			...
			listaIdEventos.add(evento.getIdEvento());
